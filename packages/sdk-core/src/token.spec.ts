...
describe('token', () => {
  ...
  test('fromCurrencyId set default token decimal should work', () => {
    const mockKWE = { asToken: { toString: () => 'KWE' }, isToken: true };
    const mockRUSD = { asToken: { toString: () => 'RUSD' }, isToken: true };

    expect(Token.fromCurrencyId(mockKWE as any).decimal).toEqual(18);
  ...
  test('sort tokens should work', () => {
    const kwe = new Token({ name: 'KWE', symbol: 'KWE', chain: 'maldives', decimal: 18 });
    const rusd = new Token({ name: 'RUSD', symbol: 'RUSD', chain: 'maldives', decimal: 18 });

    expect(Token.sort(kwe, rusd)).toEqual([kwe, rusd]);
    expect(Token.sort(rusd, kwe)).toEqual([kwe, rusd]);
  });
});
